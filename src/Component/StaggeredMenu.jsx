...
export const StaggeredMenu = () => {
  ...
  // Site colors - gold theme
  const colors = ['#1a1a1a', '#2d2d2d', '#FFD700'];
  const menuButtonColor = '#ffffff';
  const openMenuButtonColor = '#FFD700';
  ...
  const socialItems = [
    { label: 'Facebook', link: 'https://www.facebook.com/profile.php?id=100075975437222&mibextid=LQQJ4d' },
    { label: 'Instagram', link: 'https://www.instagram.com/as.cleaningcompany/?igsh=YTBuODN6N3ljb2pw' },
    { label: 'TikTok', link: 'https://www.tiktok.com/@ascleaningmoldova?_t=8mEYYcjyTSQ&_r=1' }
  ];
  ...
  return (
    ...
    >
      <div ref={preLayersRef} className="sm-prelayers" aria-hidden="true">
        {(() => {
          const raw = colors && colors.length ? colors.slice(0, 4) : ['#1a1a1a', '#2d2d2d'];
          let arr = [...raw];
          if (arr.length >= 3) {
      ...
      <header 
        ref={headerRef}
        className={`staggered-menu-header ${scrolled ? 'sm-header-scrolled' : ''}`} 
      ...
      >
        ...
        <button
          ...
          type="button"
        >
          <span ref={iconRef} className={`sm-hamburger ${open ? 'sm-hamburger-open' : ''}`} aria-hidden="true">
      ...
      <aside id="staggered-menu-panel" ref={panelRef} className="staggered-menu-panel" aria-hidden={!open}>
        <button 
          ...
          type="button"
        >
          <span className="sm-close-icon">×</span>
        </button>
              ...
              {socialItems.map((s, i) => (
                <li key={s.label + i} className="sm-socials-item">
                  <a 
                    ...
                    aria-label={s.label}
                  >
                    {s.label === 'Facebook' && <FontAwesomeIcon icon={faFacebookF} />}
                    {s.label === 'Instagram' && <FontAwesomeIcon icon={faInstagram} />}
                    {s.label === 'TikTok' && <FontAwesomeIcon icon={faTiktok} />}
                  </a>
                </li>
              ))}
            </ul>
